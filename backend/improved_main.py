...
# Load mô hình YOLO với cải thiện
def load_model():
    try:
        # Thử load các model đã train
        model_paths = [
            '../runs/train/ip102_quick_improved/weights/best.pt',
            ...
            'yolov8s.pt',  # Sử dụng YOLOv8s thay vì YOLOv8n
            'yolov8n.pt'   # Fallback
        ]
        ...
        logger.warning("Không tìm thấy model đã train, sử dụng YOLOv8s mặc định")
...
async def predict_insect(file: UploadFile = File(...)):
    """
    Nhận diện côn trùng từ ảnh upload với cải thiện
    """
    try:
        # Kiểm tra file
        ...
        results = model(image_array, conf=0.25, iou=0.45)  # Tăng confidence threshold
        processing_time = time.time() - start_time
        
        # Xử lý kết quả với cải thiện
        predictions = []
        for result in results:
            ...
            if boxes is not None:
                for box in boxes:
                    ...
                    # Chỉ giữ lại predictions có confidence cao
                    if conf < 0.3:  # Tăng threshold
                        continue
